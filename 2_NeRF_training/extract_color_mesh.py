...
if __name__ == "__main__":
    ...
    assert(y_length > 0)
    assert(z_length > 0)
    length_max = sorted([x_length, y_length, z_length])[-1]
    x_scale = x_length/length_max
    y_scale = y_length/length_max
    z_scale = z_length/length_max
    x_interval_num = int(x_scale*N)
    y_interval_num = int(y_scale*N)
    z_interval_num = int(z_scale*N)
    x = np.linspace(xmin, xmax, x_interval_num)
    y = np.linspace(ymin, ymax, y_interval_num)
    z = np.linspace(zmin, zmax, z_interval_num)
    ...
    sigma = np.maximum(sigma, 0).reshape(y_interval_num, x_interval_num, z_interval_num)
    ...
    vertices[:,0] /= x_interval_num
    vertices[:,1] /= y_interval_num
    vertices[:,2] /= z_interval_num
    ...
    vertices_ = np.zeros_like(vertices)
    vertices_[:, 0] = (ymax-ymin) * vertices[:, 1] + ymin
    vertices_[:, 1] = (xmax-xmin) * vertices[:, 0] + xmin
    vertices_[:, 2] = (zmax-zmin) * vertices[:, 2] + zmin
    ...
    PlyData([PlyElement.describe(vertices_[:, 0], 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'{args.root_dir}/{args.scene_name}.ply')

    # remove noise in the mesh by keeping only the biggest cluster
    print('Removing noise ...')
    mesh = o3d.io.read_triangle_mesh(f"{args.root_dir}/{args.scene_name}.ply")
